...
class VNSeqMCustomInit(BaseSchema):
    """
    Add views sequentially with custom initial views
    """
    def __init__(self, dataset_size: int, scene, N: int=20, M: int=1, 
                 initial_views: List[int]=None, interval_epochs=100, **kwargs):
        """
        N: int total views to select
        M: # views to select each time
        initial_views: list of initial view indices to use
        """
        super().__init__()
        
        # 使用提供的初始视角列表，如果未提供则使用[0]
        self.init_views = initial_views if initial_views is not None else [0]
        self.load_its = {}
...
# 使用partial创建预定义配置
V20Seq1Custom = partial(VNSeqMCustomInit, N=20, M=1, initial_views=[0, 24, 49, 74])
...
schema_dict: Dict[str, BaseSchema] = {'all': All, "debug": V20Seq1Debug,
                                      ...
                                      "v20seq4_inplace": V20Seq4Inplace, "v20seq1_custom": V20Seq1Custom
                                      }
