...
target_path = str(Path(__file__).parent.parent.parent / "naive_baseline_export.py")
sys.path.append(str(Path(target_path).parent))  # 添加目标所在目录到路径

# 现在可以导入
from naive_baseline_export import load_models, preprocess_data, generate_interpolated_views, extract_frames_from_video, parse_nerf_json_with_idxs
...
def generate(idxs):
    json_path = "/openbayes/input/input0/datasets/nerf/nerf_synthetic/chair/transforms_train-rgb.json"
    output_dir = "/openbayes/input/input0/datasets/tmp-chair"
    ...
    for frame_path in frame_paths:
        ...
        gt_image = F.interpolate(
            gt_image.unsqueeze(0),  # 添加batch维度 [1, 3, H, W]
            size=(800, 800),        # 目标尺寸
            mode='bilinear',        # 双线性插值
            align_corners=False     # 对齐方式
        ).squeeze(0)                # 移除batch维度 [3, 800, 800]
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if num_views > 0:
            ...
            for selected_view in selected_views:
                print("here")
                print(scene.train_cameras[1][selected_view].original_image.size())
                scene.train_cameras[1][selected_view].original_image = generate(scene.train_idxs)[0]
